...
func CreateUser(user users.User) (*users.User, *errors.RestErr) {
	...
	if err != nil {
		return nil, errors.NewBadRequestError("failed to encrypt passwrod")
	}
...
func GetUser(user users.User) (*users.User, *errors.RestErr) {
	...
	resultWp := &users.User{ID: result.ID, FirtName: result.FirtName, LastName: result.LastName, Email: result.Email}
	return resultWp, nil
}
